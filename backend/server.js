...
const { addUser, removeUser, getUserByUsername, getUser, getUsersInRoom } = require('./users');
...
io.on('connection', (socket) => {
  ...
  socket.on('privateMessage', ( username, PrivateChatSecondUsername, message, callback ) => {
    const user = getUserByUsername(PrivateChatSecondUsername);
     // sending to individual socketid (private message)
    io.to(user.id).emit('privateMess', { user: username, text: message});
    io.to(socket.id).emit('privateMess', { user: username, text: message});
    callback();
